...
/**
 * This method is responsible for create mongo full text search using filed
 * @param {array} dollarQuery dollar query
 * @returns {*} where
 */
const fullTextSearchQueryHandler = async (dollarQuery) => {
  const where = {};

  /* Filter $search from query */
  const searchQuery = await filterByEqString(dollarQuery, '$fq');

  /* Return if dollarQuery has not a $fq key */
  if (!(searchQuery && searchQuery.$fq)) return where;

  /* Break multiple $fq filed with , operator */
  const searchQueryArray = searchQuery.$fq.split('|');
  if (searchQueryArray.length <= 1) return where;

  const text = searchQueryArray[0];
  const fields = searchQueryArray[1].split(',');
  const query = searchQueryWithFields(fields, text, true);
  if (query.length > 0) where.$or = query;
  return where;
};
