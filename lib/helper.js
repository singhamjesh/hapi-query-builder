 ...
 * @returns {*} value
 */
const handleType = (value, isRegex = false, i = false, fullTextSearch=false) => {
  if (ObjectId.isValid(value) && value.length === 24) {
    return ObjectId.createFromHexString(value);
  }
  ...
  const pattern = fullTextSearch ? `\\b${text}\\b` : text;
  if (isRegex && i) {
    return {
 ...
 */
const searchQueryWithFields = (fields, q, fullTextSearch=false) => {
  return fields.map((field) => {
    return { [field]: handleType(q, true, true, fullTextSearch) };
  });
};
